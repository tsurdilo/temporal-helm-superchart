	...
	app.Commands = []*cli.Command{
		...
		},
		{
			...
			Action: func(c *cli.Context) error {
				...
				// Build in-cluster K8s client for ConfigMap dynconfig
				k8sCfg, err := rest.InClusterConfig()
				if err != nil {
					return cli.Exit(fmt.Sprintf("Unable to build in-cluster K8s config: %v.", err), 1)
				}
				k8sClient, err := kubernetes.NewForConfig(k8sCfg)
				if err != nil {
					return cli.Exit(fmt.Sprintf("Unable to create K8s client: %v.", err), 1)
				}
				...
				dcClient, err := configmapdynconfig.NewClient(
					...
					k8sClient,
					dynCfg.Namespace,
					dynCfg.ConfigMapName,
					dynCfg.DataKey,
					logger,
					metricsHandler.WithTags(metrics.StringTag("service_name", strings.Join(services, ","))),
				)
				if err != nil {
					return cli.Exit(fmt.Sprintf("Unable to create configmap dynconfig client: %v.", err), 1)
				}
				defer dcClient.Stop()
